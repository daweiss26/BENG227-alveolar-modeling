...
class PerfusionGasExchangeModel():
    ...
    def GasExchange(self, domain, guess=None, save=True, plot=True, p_val = 1, postprocess=True, plot_lines=True, verbose=True):
        ...
        S_Tot = S_HbO2 + S_HbCO2
        print('FINISHED')
        O2_avg = domain_average(domain, S_HbO2)
        CO2_avg = domain_average(domain, S_HbCO2)
        print(f"domain average S_HbO2 = {O2_avg}")
        print(f"domain average S_HbCO2 = {CO2_avg}")  
        if (O2_avg > 0 or CO2_avg > 0) and (O2_avg + CO2_avg) > 1:
            print('NORMALIZING')
            S_HbO2 /= S_Tot 
            S_HbCO2 /= S_Tot # OR I CAN USE THE AVG SUM
            print('NORMALIZED VALUES:')
            print(domain_average(domain, S_HbO2))
            print(domain_average(domain, S_HbCO2))      
